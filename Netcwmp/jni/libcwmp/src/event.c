#include "libcwmp/include/types.h"
...
int cwmp_event_global_init(cwmp_t * cwmp)
{
    ...
    {
    	printf("access file name is %s\n",cwmp->event_filename);
        return CWMP_ERROR;
    }
    printf("cwmp_event_global_init file name is %s\n",cwmp->event_filename);
...
int cwmp_event_init(cwmp_t *cwmp)
{
    ...
    cwmp->el = cwmp_event_list_create(cwmp->pool, INFORM_MAX);//创建事件链表为最大值
