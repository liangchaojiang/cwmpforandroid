             ...
             tchar_t *buffer, int buffer_size, const tchar_t *filename)
{
    ...
    if (ini_openread(filename, &fp))
    {
        ...
        if(strncmp(key,"cpe_manufacture",15)==0)
        {
        	...
        	strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_oui",7)==0)
        {
        	char osVersion[PROP_VALUE_MAX];
        	int osVersionLength = __system_property_get("ro.product.manufactureroui",osVersion);
        	if(osVersionLength!=0)
        	strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_sn",6)==0)
        {
        	char osVersion[PROP_VALUE_MAX];
        	int osVersionLength = __system_property_get("ro.product.stb.serialnum",osVersion);
        	if(osVersionLength!=0)
        	strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_name",8)==0)
        {
             char osVersion[PROP_VALUE_MAX];
             int osVersionLength = __system_property_get("ro.product.name",osVersion);
             if(osVersionLength!=0)
             strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_pc",6)==0)
        {
             char osVersion[PROP_VALUE_MAX];


             int osVersionLength = __system_property_get("ro.product.productclass",osVersion);
             //if(osVersionLength==0)strncpy(osVersion,"ProduClassNull",sizeof("ProduClassNull")-1);
             if(osVersionLength!=0)
             strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_stbid",9)==0)
        {
             char osVersion[PROP_VALUE_MAX];
             int osVersionLength = __system_property_get("ro.stbId",osVersion);
             if(osVersionLength!=0)
             strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_specver",11)==0)
        {
             char osVersion[PROP_VALUE_MAX];
             int osVersionLength = __system_property_get("ro.vendor.sw.version",osVersion);
             if(osVersionLength!=0)
             strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_hwver",9)==0)
        {
             char osVersion[PROP_VALUE_MAX];
             int osVersionLength = __system_property_get("ro.build.hardware.id",osVersion);
             if(osVersionLength!=0)
             strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_version",11)==0)
        {
             char osVersion[PROP_VALUE_MAX];
             int osVersionLength = __system_property_get("ro.product.version",osVersion);
             strncpy(buffer,osVersion,sizeof(osVersion)-1);
        }

        else if(strncmp(key,"cpe_summary",11)==0)
        {
        	cwmp_log_debug("Now in ini_gets cpe_summary\n");
             char osVersion[PROP_VALUE_MAX];
             int osVersionLength = __system_property_get("ro.devicesummary",osVersion);
             cwmp_log_debug("osVersion summary is %s\n", osVersion);
             if(osVersionLength!=0)
             strncpy(buffer,osVersion,sizeof(osVersion)-1);
             cwmp_log_debug("osVersion summary buffer is %s\n", buffer);
        }
        else;
#endif
        ini_close(&fp);
