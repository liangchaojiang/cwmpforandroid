...
#include "libcwmp/include/buffer.h"
#include "libcwmp/include/log.h"
#include "libcwmp/include/event.h"
...
int  cwmp_parse_setparametervalues_message(env_t * env , xmldoc_t * doc, parameter_node_t * root, parameter_list_t ** ppl, fault_code_t *fault)
{
    ...
    *ppl = cwmp_create_parameter_list(env);
    ...
    while (parameterNode)
    {
        ...
        {
            //faild
        	cwmp_log_debug("cwmp_create_parameter failed");
        	continue;
        }
        parameter_node_t * pn = cwmp_get_parameter_node(root, name);
        if (!pn)
        {
            ...
            cwmp_log_debug("can not find parameter %s.", name);
            cwmp_log_error("can not find parameter %s.", name);
            continue;
        }
        else
        {
            ...
            {
            	cwmp_log_debug("parameter->fault_code != FAULT_CODE_OK");
